...
```
- p valor significante, menor que 5% do nível
- coeficiente para a glicose (0.02196) positivo,  ou seja, quanto maior o nível de glicose, maior os outros atributos
```{r}
...
- hipotese alernativa: a verdadeira correlação não é 0, diferente de 0 estatisticamente
- hipotese nula: a correlação é igual a 0 estatisticamente
...
```
quanto menor, melhor. o melhor é o da 3° coluna
AIC = AKAIKE
BIC = SCHWARZ
...
rejeita por ter p-value menor que 5% (0,05)
```{r}
...
```{r}
library(nlme)
reg.gls <- gls(log(salary) ~ top10 + r11_25 + r26_40 + r41_60 + r61.100 + log(LSAT) + log(GPA) + log(libvol) + log(cost), data = lawsch85, na.action = na.exclude)
